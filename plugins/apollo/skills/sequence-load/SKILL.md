...
Use `mcp__apollo__apollo_mixed_people_api_search` with the targeting criteria.
...
1. **Enrich** — Use `mcp__apollo__apollo_people_bulk_match` (batch up to 10 per call) with:
   - `first_name`, `last_name`, `domain` for each person
   - `reveal_personal_emails` set to `false` unless the user explicitly requested and confirmed personal email retrieval before the approved enrichment step

2. **Create contacts** — For each enriched person, use `mcp__apollo__apollo_contacts_create` with:
...
3. **Remove a contact** — Show the exact contacts and sequence, require fresh confirmation, then use `mcp__apollo__apollo_emailer_campaigns_remove_or_stop_contact_ids`
