...
- Filter by sequence name → first call `mcp__apollo__apollo_emailer_campaigns_search` to resolve the name to an ID, then pass `filters: { emailer_campaign_ids: ["<id>"] }`
...
Use `mcp__apollo__apollo_analytics_sync_report` with the parameters determined above.
