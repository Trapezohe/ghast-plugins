...
Use `mcp__apollo__apollo_mixed_companies_search` with the company filters:
...
Use `mcp__apollo__apollo_organizations_bulk_enrich` with the domains from the top 10 results. This reveals revenue, funding, headcount, and firmographic data to help rank companies.
...
> **Credit confirmation**: Tell the user exactly how many credits will be consumed and wait for explicit confirmation before proceeding.

Use `mcp__apollo__apollo_people_bulk_match` to enrich up to 10 leads per call with:
- `first_name`, `last_name`, `domain` for each person
- `reveal_personal_emails` set to `false` by default. Use `true` only when the user explicitly requests personal email data, confirms that it is necessary, and the use complies with applicable policy and law.
...
1. **Save all to Apollo** — Bulk-create contacts via `mcp__apollo__apollo_contacts_create` with `run_dedupe: true` for each lead
