...
fixed list, then aggregate with `aggregate_deals` — never tally deals by
hand from `search_deals`.
...
For unresolved contact/user IDs, call `fetch_contact_by_id` on the top few only.
