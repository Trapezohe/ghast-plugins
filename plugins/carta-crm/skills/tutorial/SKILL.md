...
Companies also support **enrichment** — paste in a domain and the agent
...
notes about last week's meeting". the agent picks the right action automatically.
...
**The key shortcut:** the agent holds context across a conversation — once
...
the agent knows exactly which record you mean.
...
Say: "sign in to Carta CRM" and the agent will give you a URL to open in your browser.
...
the agent will search and show you a table of matches. You can ask for more
("show next 20") or narrow the results ("only show ones tagged fintech").

**Adding records:**

Say "add a new investor [name]" or "create a deal for [company]".
the agent will ask for anything it needs — like which pipeline and stage for a
...
the agent will show you what's there now, apply your changes, and confirm.
...
| 2 | Add a new deal | the agent proposes defaults — you confirm |
...
**the agent searches and finds 2 matches:**
...
**the agent asks:** "Found 2 investors matching 'Redwood'. Want to see the
...
> the agent will say no matches were found and suggest trying a broader term or
...
### Demo Scenario 2 of 4 — the agent proposes defaults, you confirm
...
the agent checks what pipelines Meridian has configured, then confirms before
saving anything:
...
**the agent tells you:** "Done — Apex Analytics is now in Deal Flow → Intro
...
> **Why this matters:** the agent always confirms the pipeline and stage before
> creating anything, so a deal never ends up in the wrong place. If you mention
> a stage that doesn't exist, the agent shows you your available stages and asks
...
> After the core fields, the agent will ask about any custom fields your team
...
the agent pulls the title and content straight from your message — no
formatting needed:
...
**the agent tells you:** "Note saved. Find it later with 'search notes about
...
> just describe the meeting in plain language and the agent figures out the title
...
> Just say "update my note about Sarah Chen at Apex Analytics" — the agent will
> find the note, show you what's there, and apply whatever changes you describe.
> If you have multiple notes that match, the agent will ask which one you mean.
>
> Ready for scenario 4?
...
the agent finds the Apex Analytics record and pulls in public data for that domain.
...
**the agent tells you:** "Done — company record updated with description,
...
> typing company details. the agent shows you the preview before writing anything,
...
> the agent will tell you nothing was found and won't touch the record. Try the
...
Chen as a contact at Apex Analytics". the agent handles the rest.
