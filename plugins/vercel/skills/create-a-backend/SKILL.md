...
metadata:
  ...
  pathPatterns: '["Dockerfile.vercel","Containerfile.vercel"]'
  promptSignals: '{"allOf":[["backend","vercel"],["backend","docker"],["backend","node"],["backend","python"],["backend","queue"]],"anyOf":["api","database","service","workflow"],"minScore":6,"noneOf":[],"phrases":["backend on vercel","vercel backend","build a backend","create a backend","deploy my backend","backend architecture"]}'
  summary: Match backend workloads to the right architecture
  retrieval: '{"aliases":["Vercel backend","backend architecture","backend framework"],"entities":["Vercel Functions","Vercel Services","Vercel Queues","Vercel Workflow","Vercel Marketplace","container images"],"intents":["build a backend","choose backend products","deploy an existing API or server","select a Node.js or Python backend framework"]}'
---
# Create a Backend
