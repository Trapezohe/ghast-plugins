...
>
> **Obtain any host-required approval for Catalyst CLI commands that need network access or home-directory/global CLI writes.** This includes `catalyst login`, `catalyst init`, `catalyst deploy`, commands that install/update dependencies, and commands that change global CLI config such as `catalyst config:set`. Explain that the command may contact Catalyst/Zoho services and write credentials, project links, or CLI config outside the workspace.
...
- Guide them to the bundled Ghast MCP bridge first, or to their host's MCP settings if they are using another MCP-capable client
...
| Reference | Contents |
|-----------|----------|
| `references/preflight.md` | **Workspace readiness gate** — the canonical pre-flight every skill links to: establish + reconcile org/project (`.catalystrc` ↔ MCP via `Get_Project_By_Id`), environment awareness, including Ghast's explicit no-hook readiness flow |
...
| `references/setup/codex.md` | Installing the Ghast plugin and connecting Zoho MCP in Ghast |
