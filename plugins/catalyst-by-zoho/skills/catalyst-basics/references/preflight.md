...
> **Host execution approval:** `catalyst init` links the workspace to a remote Catalyst project and
> writes CLI project/config files. Obtain any approval required by the host before running it, explaining that it
> may contact Catalyst/Zoho services and write project links or CLI config outside the workspace.
...
## Ghast readiness without the SessionStart hook
