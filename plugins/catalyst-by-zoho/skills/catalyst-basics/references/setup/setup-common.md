...
Shared across Ghast and MCP-capable IDEs (Claude Code, Cursor, GitHub Copilot). Your client-specific setup file links here for these steps and adds only what differs (skill activation, MCP client config path/dialect, and client-specific troubleshooting).
...
Then use the bundled Ghast bridge or add the URL to another client's MCP config — see **Step 2** in your client-specific setup file (`codex.md` (Ghast adaptation), `claude-code.md`, `cursor.md`, or `github-copilot.md`). The config path and JSON dialect differ per client.
