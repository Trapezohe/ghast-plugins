...
> **Host execution approval:** Obtain any approval required by the host before running Catalyst CLI commands that require network access or write outside the workspace, including `catalyst login`, `catalyst init`, `catalyst deploy`, dependency install/update flows, and global CLI config changes such as `catalyst config:set`. In the approval text, state that the command may contact Catalyst/Zoho services and write credentials, project links, or CLI config in the user's home directory.
