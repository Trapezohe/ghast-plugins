---
name: "replay-qa"
description: "Use when running Replay QA against production, staging, PR preview, or other reachable app URLs from Ghast. Provides packaged scripts for project creation/reuse, status, bugs, journeys, test runs, explorations, and bug-fix verification."
---
...
This Ghast package is for QA runs against production, staging, PR previews, and other reachable app URLs. Browser video evidence and Replay.io MCP debugging belong to the `replayio` / `codex-pro` package.
...
When Ghast lists this skill, use that listed filesystem path for `SKILL_DIR`. Do not guess a project-local path unless this package was installed into the current project.
