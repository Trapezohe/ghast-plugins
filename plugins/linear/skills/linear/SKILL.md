...
This skill provides a structured workflow for managing issues, projects & team workflows in Linear. It uses the Linear MCP connection installed with this plugin.
...
### Step 0: Connect Linear (if not already configured)

If Linear tools are unavailable, pause and ask the user to connect Linear:

1. Open Settings → MCP and authorize the `linear:linear` connection.
