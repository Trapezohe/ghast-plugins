...
This package runs a security-rebuilt bundle from ZoomInfo's exact official v1.0.1 TypeScript source. Only compatible dependency resolutions changed during the build; run every example through the bundled launcher and do not install or upgrade a different CLI version during a task.

```bash
node skills/gtm-ai-cli/scripts/gtm.mjs --version
```
...
To revoke the current session and start a fresh approved OAuth registration (rare):

```bash
node skills/gtm-ai-cli/scripts/gtm.mjs auth logout
node skills/gtm-ai-cli/scripts/gtm.mjs auth login
```

## The `lookup` workflow — read this first

ZoomInfo's search endpoints reject free-text industry/metro/topic strings with HTTP 422. **Always look up the canonical ID first**, then pass it to the search filters. Treat `node skills/gtm-ai-cli/scripts/gtm.mjs lookup` as a required first step for any filtered search.

```bash
# Industries containing "software"
node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field industries --fuzzy software -f table

# Management levels (canonical names: Board Member, C Level Exec, VP Level Exec, Director, Manager, Non Manager)
node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field management-levels -f table

# Intent topics
node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field intent-topics --fuzzy cloud -f table

# Tech products require TWO lookups: vendor first, then products under that vendor
node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field tech-vendors --fuzzy hubspot
node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field tech-products --vendor "HubSpot, Inc"

# Metro regions (US + Canada only)
node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field metro-regions --fuzzy "san francisco"
```
...
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --industry software --metro "CA - San Francisco" --employees "100to249,250to499"
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --type public --country "United States" --revenue-min 1000000 --sort -revenue --page-size 10
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --tech "<tech_product_id>" --metro "MA - Boston"
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --naics "541511,541512"
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --id 344589814
node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --domain stripe.com
node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --name "Stripe"
node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --ticker NVDA
node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --domain stripe.com --fields name description website revenue employeeCount industries

# Bulk: up to 10 via --file (JSON array of identifier objects)
node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --file ./companies.json
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs companies similar --id 344589814
node skills/gtm-ai-cli/scripts/gtm.mjs companies similar --name "Stripe"   # less accurate; CLI resolves a best-match company first
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --first-name Henry --last-name Schuck
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --job-title "CFO OR VP Finance OR Treasurer" --employees "1000to4999,5000to9999"
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --management-level "C Level Exec,VP Level Exec" --company-id 344589814
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --management-level Manager --department "Engineering & Technical" --industry software --metro "CA - San Francisco" --required email,phone
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --management-level "VP Level Exec" --department Sales --accuracy-min 90
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich --id 1260398587                                      # most accurate
node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich --email henry@zoominfo.com
node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich --phone 555-303-1234
node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich --first-name Henry --last-name Schuck --company "ZoomInfo"
node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich --full-name "Henry Schuck" --company-id 344589814
node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich --email jane@acme.com --fields phone jobTitle managementLevel email

# Bulk: up to 10 via --file
node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich --file ./contacts.json
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs contacts similar --person-id 1260398587
node skills/gtm-ai-cli/scripts/gtm.mjs contacts similar --person-id 1260398587 --company-id 239305146   # similar contacts within Salesforce
```

**Recommended contacts** at a target company, based on your account's interaction history:

```bash
node skills/gtm-ai-cli/scripts/gtm.mjs contacts recommended --company-id 344589814 --use-case PROSPECTING
node skills/gtm-ai-cli/scripts/gtm.mjs contacts recommended --company-id 344589814 --use-case DEAL_ACCELERATION
node skills/gtm-ai-cli/scripts/gtm.mjs contacts recommended --company-id 344589814 --use-case RENEWAL_AND_GROWTH
```
...
Every intent call requires `--topics` (1-50 names — use `node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field intent-topics` to find exact names).

**Search across all companies:**

```bash
node skills/gtm-ai-cli/scripts/gtm.mjs intent search --topics "Cloud Applications" "Java" --signal-score-min 70
node skills/gtm-ai-cli/scripts/gtm.mjs intent search --topics "Mobile Apps" --signal-score-min 80 --signal-start 2026-05-01 --signal-end 2026-05-31 --industry software --country "United States"
node skills/gtm-ai-cli/scripts/gtm.mjs intent search --topics "AI Agents" --audience-strength-min B --audience-strength-max A
```

**Enrich** — fetch intent for a specific company:

```bash
node skills/gtm-ai-cli/scripts/gtm.mjs intent enrich --company-id 344589814 --topics "Cloud Applications" "Java"
node skills/gtm-ai-cli/scripts/gtm.mjs intent enrich --name "ZoomInfo" --topics "AI Agents" --signal-score-min 70
node skills/gtm-ai-cli/scripts/gtm.mjs intent enrich --website https://www.stripe.com --topics "Payments"
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs scoops search --scoop-types Funding --industry software --metro "MA - Boston" --published-start 2026-05-01 --published-end 2026-05-31
node skills/gtm-ai-cli/scripts/gtm.mjs scoops search --scoop-types "New Hire" "Executive Move" --department C-Suite --published-start 2026-05-27
node skills/gtm-ai-cli/scripts/gtm.mjs scoops search --scoop-types Layoffs --employees "10000plus" --published-start 2026-01-01
node skills/gtm-ai-cli/scripts/gtm.mjs scoops search --scoop-types "Product Launch" --description "artificial intelligence" --published-start 2026-04-01
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs scoops enrich --company-id 344589814 --scoop-types "New Hire" Promotion
node skills/gtm-ai-cli/scripts/gtm.mjs scoops enrich --name "Stripe" --published-start 2026-01-01
node skills/gtm-ai-cli/scripts/gtm.mjs scoops enrich --websites https://www.zoominfo.com https://www.salesforce.com
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs news enrich --company-id 344589814 --categories FINANCIAL_RESULTS FUNDING
node skills/gtm-ai-cli/scripts/gtm.mjs news enrich --company-id 344589814 --categories PRODUCT --publishing-start 2026-03-01 --publishing-end 2026-05-31
node skills/gtm-ai-cli/scripts/gtm.mjs news enrich --company-id 344589814 --categories PERSON
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs gtm-context get                                                     # compressed (~1k tokens)
node skills/gtm-ai-cli/scripts/gtm.mjs gtm-context get --detailed -f yaml                                  # full expanded profiles
node skills/gtm-ai-cli/scripts/gtm.mjs gtm-context update --query "Update competitive intelligence" --source "Our main competitors are X, Y, Z…"
node skills/gtm-ai-cli/scripts/gtm.mjs gtm-context update --query "Create buyer personas from these interviews" --source-file ./interview-notes.txt
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs feedback submit --category DATA_QUALITY --message "Henry Schuck's company should be ZoomInfo Technologies, not ZoomInfo"
node skills/gtm-ai-cli/scripts/gtm.mjs feedback submit --category FEATURE_REQUEST --message "Want a --csv-headers flag to control column order"
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs raw list-tools -f table                                                          # show every MCP tool the server exposes
node skills/gtm-ai-cli/scripts/gtm.mjs raw call account_research --args '{"companyId":"344589814"}'
node skills/gtm-ai-cli/scripts/gtm.mjs raw call contact_research --args '{"personId":"1260398587"}'
node skills/gtm-ai-cli/scripts/gtm.mjs raw call find_similar_companies --args '{"companyName":"Stripe"}'
```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --industry software -f table
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --management-level "C Level Exec" -f jsonl > c-level-contacts.jsonl
node skills/gtm-ai-cli/scripts/gtm.mjs gtm-context get -f yaml
```
...
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --industry software --metro "CA - San Francisco" \
  | jq -r '.data[].attributes.name'

# Sorted by revenue, top 5 names + revenue
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --type public --country "United States" --revenue-min 1000000 --page-size 50 \
  | jq '[.data[] | { name: .attributes.name, revenue: .attributes.revenue }] | sort_by(-.revenue) | .[0:5]'

# Email + phone TSV for every contact result
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --company-id 344589814 -f jsonl \
  | jq -r '[.attributes.email, .attributes.phone] | @tsv'

# Top 5 intent signals by score
node skills/gtm-ai-cli/scripts/gtm.mjs intent search --topics "Cloud Applications" --signal-score-min 80 \
  | jq '.data | sort_by(-.attributes.score) | .[0:5]'

# Lookup → extract IDs → search
INDUSTRY_IDS=$(node skills/gtm-ai-cli/scripts/gtm.mjs lookup --field industries --fuzzy software | jq -r '.industries.data[].id' | paste -sd, -)
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --industry "$INDUSTRY_IDS" --metro "CA - San Francisco"
```
...
>     ```bash
>     node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --file ./companies.json   # up to 10 in one round trip
>     node skills/gtm-ai-cli/scripts/gtm.mjs contacts enrich  --file ./contacts.json
>     ```
...
>     ```bash
>     node skills/gtm-ai-cli/scripts/gtm.mjs companies search --industry "$IDS" --page-size 20 \
>       | jq -r '.data[].id' \
>       | xargs -P4 -I{} node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --id {}     # max 4 in-flight
>     # or: ... | parallel -j4 node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --id {}
>     ```
...
```bash
node skills/gtm-ai-cli/scripts/gtm.mjs companies search --industry software --metro "CA - San Francisco" --page-size 3 \
  | jq -r '.data[].id' \
  | xargs -I{} node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --id {}
```
...
node skills/gtm-ai-cli/scripts/gtm.mjs companies similar --id 344589814 \
  | jq -r '.data[0:10][].id' \
  | xargs -I{} node skills/gtm-ai-cli/scripts/gtm.mjs companies enrich --id {} --fields name domain employeeCount revenue industries
```

### Export to spreadsheet

```bash
node skills/gtm-ai-cli/scripts/gtm.mjs contacts search --management-level "VP Level Exec" --department Sales --company-id 344589814 -f csv > vp-sales.csv
```
...
node skills/gtm-ai-cli/scripts/gtm.mjs intent search --topics "Cloud Applications" "Data Warehouse" --signal-score-min 80 --signal-start "$(date -v-7d +%Y-%m-%d)" \
...
| `node skills/gtm-ai-cli/scripts/gtm.mjs gtm-context get` | `.result` | `{ user, organization, buyerPersonas:[…], idealCustomerProfiles:[…], competitors:[…], offerings:[…] }` |
| `node skills/gtm-ai-cli/scripts/gtm.mjs gtm-context update` | `.result` | varies by update operation |
...
- **Search commands refuse to run with zero filters.** Running e.g. `node skills/gtm-ai-cli/scripts/gtm.mjs companies search` with no flags exits with a guidance error listing common filters. This is intentional — unfiltered searches return unbounded uninteresting results.
...
- **Write confirmation:** `gtm-context update` and `feedback submit` change external state or submit data to ZoomInfo. Show the exact payload and obtain explicit user confirmation immediately before calling either command. Never retry an ambiguous result without reading current state.
