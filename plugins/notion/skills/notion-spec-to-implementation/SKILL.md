...
3) Create a plan page with `notion-create-pages` (pick a template: quick vs. full).
4) Find the task database, confirm schema, then create tasks with `notion-create-pages`.
5) Link spec ↔ plan ↔ tasks; keep status current with `notion-update-page`.
...
- To append an implementation section to a spec, fetch the current section text first, then use `notion-update-page` with `command: "update_content"`, `properties: {}`, and exact `old_str` / `new_str` content. For property-only edits, use `command: "update_properties"` with `content_updates: []`; the current deployed schema expects both top-level fields even when one is unused.
...
- Create the plan via `notion-create-pages`, include: overview, linked spec, requirements summary, phases, dependencies/risks, and success criteria. Link back to the spec.
...
- Create pages with `notion-create-pages` using the database’s `data_source_id`.

### 4) Link artifacts
- Plan links to spec; tasks link to both plan and spec.
- Optionally update the spec with a short “Implementation” section pointing to the plan and tasks using `notion-update-page`.
